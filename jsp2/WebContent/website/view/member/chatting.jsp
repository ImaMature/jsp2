<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Insert title here</title>
</head>
<body>
	<%@include file="../header.jsp" %>
	<h3 class="text-center">실시간 채팅</h3>
	<div class="container">
		<div class="row">
		
			<div class="col-md-6">
				<!-- 채팅창 -->
				<div id="msgbox">  
				 	
					<div class="d-flex justify-content-end mx-2 my-2"> 
						<span class="msgtime d-flex align-items-end">오전 9:59</span>
						<span class="from mx-1">뭐함?</span>
					</div> 
					<div class="d-flex justify-content-end mx-2 my-2"> <!-- 보내는 메시지 (오른쪽) -->
						<span class="msgtime d-flex align-items-end">오전 9:59</span>
						<span class="from mx-1">나 심심한데</span>
					</div>
					
					<div class="d-flex justify-content-start mx-2 my-2">			
						<span class="to mx-1">과제하느라 바쁨</span>		<!--받는 메시지 (왼쪽) -->
						<span class="msgtime d-flex align-items-end">오전 9:59</span>
					</div> 	
					
					<div class="d-flex justify-content-start mx-2 my-2">			
						<span class="to mx-1">ㄷㅇ한테 전화 ㄱ</span>		
						<span class="msgtime d-flex align-items-end">오전 9:59</span>
					</div> 	
					
					<div class="d-flex justify-content-end mx-2 my-2">
						<span class="msgtime d-flex align-items-end">오전 9:59</span>
						<span class="from mx-1">뭐함?</span>
					</div> 
					<div class="d-flex justify-content-end mx-2 my-2"> 
						<span class="msgtime d-flex align-items-end">오전 9:59</span>
						<span class="from mx-1">뭐함?</span>
					</div> 
					<div class="d-flex justify-content-end mx-2 my-2">
						<span class="msgtime d-flex align-items-end">오전 9:59</span>
						<span class="from mx-1">뭐함?</span>
					</div> 
					<div class="d-flex justify-content-end mx-2 my-2"> 
						<span class="msgcount d-flex align-items-end mx-1">1</span>
						<span class="msgtime d-flex align-items-end">오전 10:00</span>
						<span class="from mx-1">뭐함?</span>
					</div> 
					<div class="d-flex justify-content-end mx-2 my-2">
						<span class="msgcount d-flex align-items-end mx-1">1</span>
						<span class="msgtime d-flex align-items-end">오전 10:00</span>
						<span class="from mx-1">뭐함?</span>
					</div> 
					<div class="d-flex justify-content-end mx-2 my-2">
						<span class="msgcount d-flex align-items-end mx-1">1</span>
						<span class="msgtime d-flex align-items-end">오전 10:00</span>
						<span class="from mx-1">뭐함?</span>
					</div> 
					<div class="d-flex justify-content-end mx-2 my-2"> 
						<span class="msgcount d-flex align-items-end mx-1">1</span>
						<span class="msgtime d-flex align-items-end">오전 10:00</span>
						<span class="from mx-1">뭐함?</span>
					</div> 
					<div class="d-flex justify-content-end mx-2 my-2"> 
						<span class="msgcount d-flex align-items-end mx-1">1</span>
						<span class="msgtime d-flex align-items-end">오전 10:00</span>
						<span class="from mx-1">뭐함?</span>
					</div> 
						<div class="d-flex justify-content-end mx-2 my-2"> 
						<span class="msgcount d-flex align-items-end mx-1">1</span>
						<span class="msgtime d-flex align-items-end">오전 10:00</span>
						<span class="from mx-1">뭐함?</span>
					</div> 
						<div class="d-flex justify-content-end mx-2 my-2"> 
						<span class="msgcount d-flex align-items-end mx-1">1</span>
						<span class="msgtime d-flex align-items-end">오전 10:00</span>
						<span class="from mx-1">뭐함?</span>
					</div> 
						<div class="d-flex justify-content-end mx-2 my-2"> 
						<span class="msgcount d-flex align-items-end mx-1">1</span>
						<span class="msgtime d-flex align-items-end">오전 10:00</span>
						<span class="from mx-1">뭐함?</span>
					</div> 
				</div>
				<div class="row">	<!-- 채팅 입력 창, 버튼 -->
					<div class="col-md-10"> <!-- 채팅입력창 -->
						<input id="msginput" class="form-control" type="text" placeholder="내용입력">
					</div>
					<div class="col-md-2"> <!-- 버튼 -->
						<button class="form-control" onclick="send();">전송</button>
					</div>
				</div>
			</div>
			<!-- 채팅 끝 -->
			<!-- 로그인 목록 ( 친구 창 ) -->			
			<div class="col-md-6">
				보내는 사람 : <input id="loginid" class="form-control" type="text" value="<%=loginid %>">
				받는 사람 : <input class="form-control" type="text" value="all">
				접속 회원 목록
				<div id="mlist">
					
				</div>
			</div>
		</div>
	</div>
	
	<script type="text/javascript">
		//jsp는 예외처리가 다 만들어져 있어서 입출력할때 예외 처리 딱히 할필요 없음	
	
		//1. 입력창에 입력된 데이터를 가져온다.
		var msginput = document.getElementById("msginput").value;
		//값 끌고 오는지 확인
		//alert(msginput);
		
		//2. 현재 회원 아이디 가져오기
		var loginid = document.getElementById("loginid").value; //value속성이 있는 태그만(div제외) 값 이렇게 가져올 수 있음
		//alert(loginid);
		
		//3. 채팅창 가져오기
		var msgbox = document.getElementById("msgbox");
		//alert(msgbox);
		
		//4. 웹소켓 : var webSocket = new WebSocket("ws://웹ip:http 포트번호/프로젝트명/경로"); //웹 ip : localhost
		//접속 객체
		var webSocket = new WebSocket("ws://localhost:8085/jsp2/chatting");
				//자바 명령어는 jsp2/dto/chatting.java로 이동(그곳이 엔드포인트(@ServerEndpoint임)
		
		//5. 웹소켓 메소드 (평션 값 따로 없이 그냥 이벤트) 각자 어노테이션을 찾아감 (위치는 dto/Chatting.java)
		webSocket.onopen = function(event) {onOpen(event)}; //웹소켓 실행시 메소드
		webSocket.onClose = function(event) {onClose(event)}; //웹소켓 종료시 메소드
		webSocket.onmessage = function(event) {onMessage(event)}; //웹소켓 메시지 전송 메소드
		webSocket.onerror = function(event) {onError(event)}; //웹소켓 에러 발생시 메소드
		
		//6. 이벤트 메소드 정의
		function onOpen(event) {
			alert("접속되었습니다.");
		}
		
		function onClose(event) {
			alert("퇴장했습니다.");
		}
		
		/* function onMessage(event) {
			alert("메시지가 도착했습니다.");
		} */
		
		function onError(event) {
			alert("오류가 발생했습니다. [관리자에게 문의]");
		}
		
		//7. 보내는 메소드
		function send() {
			
			let today = new Date(); //js에서 현재 날짜/시간 객체
			var time = today.toLocaleTimeString(); //시간만 가져오기
			
			//입력된 문자와 날짜를 채팅별 div에 추가
			//전체 -> 큰따옴표 / class -> 작은따옴표
			msgbox.innerHTML += "<div class='d-flex justify-content-end mx-2 my-2'>	<span class='msgtime d-flex align-items-end'>"+time+"</span><span class='from mx-1'>"+msginput+"</span>	</div>"		
			
			webSocket.send(msginput); //서버로 입력한 메시지 전송
			
			msginput.value=""; //전송 후 입력한 내용을 지우기 [초기화]
			
			//스크롤이 있을 경우 스크롤 위치를 가장 아래로 이동
				//채팅창(박스권)의 스크롤 윗부분의 위치 =  채팅창(박스권)의 스크롤의 전체 높이 (스크롤을 높이만큼 내림)
			msgbox.scrollTop = msgbox.scrollHeight;	//현 스크롤의 위치를 바닥에 둠 //css에서 스크롤 바 숨기기 해놓음
		}
		
		//8. 받는 메소드
		function onMessage(event) {
			//alert("메시지가 왔습니다." + event.data);
			var msg = event.data;
			msgbox.innerHTML += "<div class='d-flex justify-content-start mx-2 my-2'><span class='to mx-1'>"+msg+"</span><span class='msgtime d-flex align-items-end'>오전 9:59</span></div>"
			msgbox.scrollTop = msgbox.scrollHeight;	//현 스크롤의 위치를 바닥에 둠

		}
		
	</script>
</body>
</html>